 ...
 *
 * Small-caps label, a large Caslon figure that counts up on load, and a gold-tint icon
 * disc pinned right. Cards with a delta carry a pill under the figure; the others leave
 * that row empty so all four stay the same height.
 */
...
const ICONS: Record<Kpi["icon"], React.ComponentType<{ className?: string; stroke?: number }>> = {
  file: IconFileText,
  eye: IconEye,
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {KPIS.map((kpi, i) => {
        ...
        return (
          <Panel key={kpi.label} index={i} hover className="group p-4">
            ...
            <div className="mt-3 h-[22px]">
              {kpi.delta && (
                <span className="inline-flex items-center gap-1 rounded-md bg-brand-tint px-2 py-1 text-[11px] font-medium text-brand-strong">
                  <IconArrowUpRight stroke={2} className="size-3.5" />
                  {kpi.delta}
                </span>
              )}
            </div>
